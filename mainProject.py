...
def transaction(identity, amount, choice):
    ...
    with open("./database/Customer/customerDatabase.txt", "r") as customer_database:
        found = False
        for line in customer_database:
            if identity == line.strip():
                found = True
                data_collector += line  # ID
                pin = customer_database.readline()  # Read PIN
                data_collector += pin  # Keep the PIN
                balance_line = customer_database.readline().strip()  # Read balance
                
                try:
                    balance = float(balance_line)  # Convert balance to float
                except ValueError:
                    print(f"Error: Could not convert balance '{balance_line}' to float.")
                    return -1  # Return an error code or handle as needed
                
                # Check for withdrawal conditions
                if choice == 2:
                    if balance - amount < 1000:  # Minimum balance check
                        print(f"Insufficient funds. Current balance: {balance}, Withdrawal amount: {amount}")
                        return -1  # Not enough balance for withdrawal
                    else:
                        balance -= amount  # Update balance for withdrawal
                else:
                    balance += amount  # Update balance for deposit
                
                # Write the updated balance back to data_collector
                data_collector += str(balance) + "\n"  # Update balance
                # Read remaining lines (customer details)
                for _ in range(8):  # Assuming there are 8 more lines of customer data
                    data_collector += customer_database.readline()
            else:
                data_collector += line  # Keep the other lines unchanged

    # Write updated data back to the database
    with open("./database/Customer/customerDatabase.txt", "w") as customer_database:
        customer_database.write(data_collector)
    
    # Log the transaction
    log_transaction(identity, transaction_type, amount)
    
    return balance

def log_transaction(identity, transaction_type, amount):
    with open("./database/Customer/transactions.txt", "a") as transaction_log:
        transaction_log.write(f"{identity},{transaction_type},{amount},{date.today()}\n")


def mini_statement(identity):
    try:
        with open("./database/Customer/transactions.txt", "r") as transaction_log:
            transactions = transaction_log.readlines()
            user_transactions = [transaction for transaction in transactions if transaction.startswith(identity)]
            last_four = user_transactions[-4:]  # Get the last 4 transactions for the specific user
            
            output_message = "Last 4 Transactions:\n"
            if last_four:
                for transaction in last_four:
                    output_message += transaction.replace(",", " | ") + "\n"
            else:
                output_message = "No transactions found for this account."
...
class welcomeScreen:
    def __init__(self, window=None):
        ...
        self.Button1 = tk.Button(self.Canvas1, command=self.selectEmployee, activebackground="#ececec",
                                 activeforeground="#000000", background="#0066CC", disabledforeground="#a3a3a3",
...
class customerMenu:
    def __init__(self, window=None):
        ...
        self.ButtonMiniStatement = tk.Button(self.Labelframe1, command=self.showMiniStatement, activebackground="#ececec",
                                             activeforeground="#000000", background="#39a9fc", borderwidth="0",
                                             disabledforeground="#a3a3a3", font="-family {Segoe UI} -size 11", 
                                             foreground="#fffffe", highlightbackground="#d9d9d9", highlightcolor="black", 
                                             pady="0", text='''Mini Statement''')
        self.ButtonMiniStatement.place(relx=0.353, rely=0.195, height=34, width=181, bordermode='ignore')
    ...
    def showMiniStatement(self):
        # Implement the logic to show the mini statement here
        statement = mini_statement(customer_accNO)  # Assuming customer_accNO is the account number
        self.printMessage(statement)
    ...
    def printMessage(self, output):
        ...
        for widget in Frame1_1_2.winfo_children():
            widget.destroy()
        # getting output_message and displaying it in the frame
        output_message = tk.Label(Frame1_1_2, text=output, background="#fffffe")
        output_message.pack(pady=20)
